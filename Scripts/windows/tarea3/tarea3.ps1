...
function agregarRegistro {
    ...
    try {
        ...
        Write-Host "El registro '$name' se ha creado correctamente." -ForegroundColor
    }
    catch {
        Write-Host "No se pudo crear el registro. Detalles: $($_.Exception.Message)" -ForegroundColor
    }
}
...
function agregarZonaPrimaria2 {
    ...
    try {
        ...
        Write-Host "La zona '$name' se ha creado correctamente." -ForegroundColor
    }
    catch {
        Write-Host "No se pudo crear la zona. Detalles: $($_.Exception.Message)" -ForegroundColor
    }
}
...
function actualizarRegistro{
    ...
    try {
        ...
        Write-Host "El registro '$name' se ha actualizado correctamente." -ForegroundColor
    }
    catch {
        Write-Host "No se pudo actualizar el registro. Detalles: $($_.Exception.Message)" -ForegroundColor
    }
}
...
function eliminarZona {
    ...
    if ($cf -match '^[Ss]$'){
        try {
            ...
            Write-Host "La zona '$name' se ha eliminado correctamente." -ForegroundColor
        }
        catch {
            Write-Host "No se pudo eliminar la zona. Detalles: $($_.Exception.Message)" -ForegroundColor
        }
    }
    else {
        Write-Host "Entendido, regresando..." -ForegroundColor
        return
    }
}
...
function eliminarRegistro {
    ...
    try {
        ...
        Write-Host "El registro '$name' se ha eliminado correctamente." -ForegroundColor
    }
    catch {
        Write-Host "No se pudo eliminar el registro. Detalles: $($_.Exception.Message)" -ForegroundColor
    }
...
function configurarEscenario {
    ...
    try {
        ...
        if ($pr -match '^[Ss]$'){
            Write-Host "Iniciando prueba..." -ForegroundColor
            pruebasDNS -zona $dom
        }
        else {
            Write-Host "Entendido, regresando..." -ForegroundColor
...
function pruebasDNS {
    ...
    Write-Host "1. Probando nslookup para $zona..." -ForegroundColor
    nslookup $zona 127.0.0.1
    
    Write-Host "`n2. Probando ping a www.$zona..." -ForegroundColor
    ping "www.$zona" -n 2
...
function verRegistroPorZonas{
    ...
    try {
        Write-Host " ------------------------ " -BackgroundColor -ForegroundColor White
        Write-Host " Registros existentes" -BackgroundColor -ForegroundColor White
        Write-Host " ------------------------ " -BackgroundColor -ForegroundColor White
        Get-DnsServerResourceRecord -ZoneName $name -RRType "A"  -ErrorAction Stop | Format-Table -AutoSize
    }
    catch {
        Write-Host "La zona '$name' no tiene registros." -ForegroundColor
    }
}

function verZonas {
    Write-Host " ------------------------ " -BackgroundColor White -ForegroundColor
    Write-Host " Zonas existentes" -BackgroundColor White -ForegroundColor
    Write-Host " ------------------------ " -BackgroundColor White -ForegroundColor
    Get-DnsServerZone   
}
...
function configuracionZona {
    ...
    Write-Host "   Menu configuracion de zona " -ForegroundColor
    Write-Host "----------------------------------" -ForegroundColor
    ...
    switch ($opc) {
        ...
        "8"{
            Write-Host "`nSaliendo..." -ForegroundColor
            return
        }
        
        default {
            Write-Host "Opcion no valida" -ForegroundColor
...
function instalacionDNS {
  Write-Host " ---------------------------- " -ForegroundColor
  Write-Host "Instalacion de DNS Server" -ForegroundColor
  Write-Host " ---------------------------- " -ForegroundColor
    ...
    if ($DNSEstado.InstallState -eq "Installed") {
        Write-Host "DNS server ya esta instalado" -ForegroundColor
    }
    else {
        Write-Host "DNS server no esta instalado, iniciando instalacion..." -ForegroundColor
        ...
        try {
            ...
            if ($result.Success) {
                Write-Host "DNS server instalado correctamente" -ForegroundColor
                Get-WindowsFeature -Name *DNS*
                Start-Sleep -Seconds 2
            }
            else {
                Write-Host "Error en la instalacion de DNS" -ForegroundColor
                return
            }
        }
        catch {
            Write-Host "Error durante la instalacion: $_" -ForegroundColor
            return
        }
    }
}
...
function verificarInstalacion {
    ...
    if ($DNSEstado.InstallState -eq "Installed") {
        Write-Host "DNS ya se encuentra instalado" -ForegroundColor
    }
    else {
        Write-Host "DNS no se encuentra instalado" -ForegroundColor
        ...
        if ($opcc -match '^[Ss]$') {
            ...
            Write-Host "Entendido, regresando al menu..." -ForegroundColor
            Start-Sleep -Seconds 2
        }
    }
}
...
function mostrarMenu {
    ...
    Clear-Host
    Write-Host "----------------------------------" -ForegroundColor
    Write-Host "   Menu  " -ForegroundColor
    Write-Host "----------------------------------" -ForegroundColor
    ...
    switch ($opcion) {
        ...
        "3" {
            ...
            if ($DNSEstado.InstallState -eq "Installed") {
                ...
                Write-Host "DNS no esta instalado. Instalelo primero." -ForegroundColor
            }
            Read-Host "`nPresiona Enter para continuar"
            mostrarMenu
        }

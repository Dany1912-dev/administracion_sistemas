...
# Moverse al directorio del script para que las rutas relativas funcionen
cd "$(dirname "$0")"
...
verificar_dependencias() {
    ...
    # Verificar sudo o root
    if [ "$EUID" -ne 0 ] && ! command -v sudo &>/dev/null; then
        echo "[ERROR] Este script necesita sudo o ejecutarse como root"
        exit 1
    fi

    # Verificar curl (lo usan prueba-fail2ban.sh y la instalación de Docker)
    if ! command -v curl &>/dev/null; then
        echo "[WARN] curl no encontrado. Instalando..."
    ...
    if ! command -v docker &>/dev/null; then
        echo "[WARN] Docker no encontrado. Instalando via zypper..."
        sudo zypper install -y docker
        sudo systemctl enable --now docker
        sudo usermod -aG docker "$USER"
        echo "[OK] Docker instalado. Continuando con permisos del grupo docker..."
        exec sg docker -c "bash \"$(readlink -f "$0")\""
    fi

    # Verificar que el daemon de Docker esté corriendo
    if ! docker info &>/dev/null 2>&1; then
        echo "[WARN] El daemon de Docker no está corriendo. Iniciando..."
        ...
        echo "[OK] Daemon de Docker iniciado"
    fi

    # Verificar permisos de Docker sin sudo
    if ! docker ps &>/dev/null 2>&1; then
        echo "[WARN] El usuario '$USER' no tiene permisos para usar Docker sin sudo."
        sudo usermod -aG docker "$USER"
        echo "[OK] Usuario '$USER' añadido al grupo 'docker'. Continuando con permisos del grupo..."
        exec sg docker -c "bash \"$(readlink -f "$0")\""
    fi

    # Verificar el plugin docker compose (v2)
    # En openSUSE el paquete se llama docker-compose y se instala como plugin de Docker CLI
    if ! docker compose version &>/dev/null 2>&1; then
        echo "[WARN] El plugin 'docker compose' no está disponible. Instalando..."
        sudo zypper install -y docker-compose
        if ! docker compose version &>/dev/null 2>&1; then
            echo "[ERROR] docker compose sigue sin estar disponible tras la instalación."
            echo "        Verifica con: sudo zypper search docker-compose"
            exit 1
        fi
